...
\begin{Demostracion}
  ...
  que es una biyección y por lo tanto el mapa inducido entre \(\bigcup B_0\) y \(\bigcup \KK_0\) es 
  un homeomorfismo.

  \underline{Paso Inductivo:} Sea \(1 \le j \le d-1\) y supongamos que \(f_j\colon \bigcup B_j \to \bigcup \KK_j\) 
  ya ha sido construido. Seguiremos el siguiente algoritmo:
  \begin{enumerate}[itemsep=1pt]
    \item Inicializar \(\KK_{j+1} =\KK_{j}\). 
    \item Escoger \(\Y\subset\X\) de dimensión \(d-(j+1)\) tal que \(F\coloneqq \bigcap V_{\Y}\cap \MM\) es no vacía
      (y tal que \(e_{\Y}\not\in\KK_{j+1}\)). 
    \item Agregar símplices a \(\KK_{j+1}\) de tal forma forma que haya un homeomorfismo \(f_{\Y}\) entre
      \(F\) y los símplices agregados.
    \item Volver al paso 2 si quedan símplices por revisar.
    \item Pegar todos los homeomorfismos \(f_{\Y}\) para construir \(f_{j+1}\).
  \end{enumerate}
  Explicación del Paso 2: 

  Definamos \(G \coloneqq \bigcap V_{\Y} \cap \partial\MM\). 
\end{Demostracion}
...
\begin{Demostracion}
  ...
  \vspace{-0.7cm}
    ...
    \begin{tikzpicture}
      ...
      \draw (0,2) circle(2cm);
      \draw[thick] (0,0) -- (0,2) node[above] {\(c_{+}\)};
  ...
  \end{align*}
  Notamos que \(\sin(2\theta) > 0\) pues \(\theta\) varía entre (cotas holgadas) \((0,\pi/2)\).
  Dado que \(\epsilon < \tau\), tenemos que
  \begin{displaymath}
    \left( 1 - \frac{\reach\MM\,\cos(2\theta)}{\sqrt{\epsilon^2 - \reach\MM^2 \sin^2(2\theta)}} \right) 
    < 0.
  \end{displaymath}
  Es decir, \(A\) es decreciente con \(\theta\). Luego, \(A\) alcanza su máximo valor
  cuando \(\theta\) es lo más chico posible. Lo más pequeño que puede ser \(\theta\) es
  hasta que \(b = 2\reach\MM \sin(\theta) = \epsilon\), pues de lo contrario \(x_i \in B_{\epsilon}(m)\). 
  Evaluando en \(A\) vemos que su máximo valor es de \(\epsilon^2/\reach\MM\). 

  Establecidas las relaciones anteriores podemos comenzar a mover \(\epsilon\) de tal forma que mantengamos
  la máxima distancia \(A\). En particular, queremos ver para qué valores de \(\epsilon\) podemos
  asegurar que el trayecto \(\overline{x\,m}\) está completamente contenido en \(\X^{\oplus\epsilon}\).
...
Consideremos \(n+1\) puntos \(a_0, \ldots, a_n\) de \(\R^n\) en posición general.
Queremos encontrar la bola que circumscribe a estos puntos. Sea \(c\) el centro de
tal bola. Luego,
\begin{displaymath}
  \norm{a_0 - c}^2 = \norm{a_i - c}^2 \quad\forall 1\le i\le n.
\end{displaymath}
Notando que \(\norm{x}^2 = \langle x,x \rangle\) tenemos que
\begin{displaymath}
  \norm{a_0}^2 + \norm{c}^2 - 2 \langle a_0, c \rangle
  =
  \norm{a_i}^2 + \norm{c}^2 - 2 \langle a_i, c \rangle
\end{displaymath}
Y por lo tanto \(c\) resuelve el sistema lineal
\begin{displaymath}
  \begin{bmatrix}
    \vdash (a_1 - a_0)^T \dashv \\
    \vdots\\
    \vdash (a_n - a_0)^T \dashv \\
  \end{bmatrix} c 
  =
  Ac
  =
...
Como los puntos están en posición general, la matriz \(A\) es invertible y siempre hay solución.
Supongamos que tenemos \(k+1 < n+1\) puntos. Luego, \(\ker A\) se compone de los vectores

\begin{wrapfigure}{r}{0.25\textwidth}
  \begin{tikzpicture}[scale=2]
    \clip(-1.1,1.4) rectangle (1.1,-0.5);
    \draw[-,gray] (0,-.5) -- (0,1.1);
    \draw[-,gray] (-1,0) -- (1, 0);
    \draw (0,.5) circle(0.5cm);
    \draw (-1.2,0.5) circle(1.3cm);
    \draw[fill=black] (0,0) circle(1pt);
    \draw[fill=black] (0,1) circle(1pt);
    \draw[fill=black] (0.1,0.5) circle(1pt);
  \end{tikzpicture}
\end{wrapfigure}

\noindent que viven en el espacio ortogonal a \(\aff(a_0, \dots, a_k)\). Más aún, el espacio
ortogonal pasa por el centro de la \(k\)-bola que circumscribe a \(a_0, \dots, a_k\) en el 
espacio afín que generan.

Notar que la circumbola no es lo mismo que la bola de cobertura mínima. 
Como contraejemplo, basta considerar los puntos \((0,0), (0,1)\) y \((0.1, 0.5)\) en \(\R^2\).
Como ilustra la imagen de la derecha, la circumbola de los tres puntos es mucho más grande.

\begin{Definicion}[Bola de Cobertura Mínima]
  Sea \(\X\subset\R^n\) un conjunto finito. La bola de cobertura mínima de \(\X\), denotada
  \(mb(\X)\) es la \(n\)-bola cerrada de menor radio tal que \(\X\subset mb(\X)\).
\end{Definicion}
